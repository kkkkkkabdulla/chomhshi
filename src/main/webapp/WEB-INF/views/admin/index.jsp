...
<head>
    ...
    <style>
        ...
        .ud-header { display:flex; align-items:center; gap:18px; padding:20px 24px; background:linear-gradient(135deg,#f0f5ff,#eef1ff); border-radius:12px; margin-bottom:20px; }
        .ud-avatar { width:64px; height:64px; border-radius:50%; background:#fff; border:3px solid #fff; box-shadow:0 2px 12px rgba(22,119,255,.15); object-fit:cover; }
        .ud-name { font-size:20px; font-weight:700; color:#1a1d2e; }
        .ud-id { font-size:13px; color:#8b93a7; margin-top:2px; }
        .ud-status { display:inline-block; padding:3px 12px; border-radius:99px; font-size:12px; font-weight:600; margin-top:6px; }
        .ud-status-ok { background:#e6f4ea; color:#1a7f4b; }
        .ud-status-ban { background:#fff1f0; color:#cf1322; }
        .ud-info-grid { display:grid; grid-template-columns:1fr 1fr; gap:12px 24px; margin-bottom:20px; }
        .ud-info-item { padding:12px 16px; background:#fafbfe; border-radius:10px; border:1px solid #f0f2f8; }
        .ud-info-label { font-size:12px; color:#8b93a7; margin-bottom:4px; font-weight:500; }
        .ud-info-value { font-size:14px; color:#1a1d2e; font-weight:600; }
        .ud-section { margin-bottom:18px; }
        .ud-section-title { font-size:14px; font-weight:700; color:#1a1d2e; padding:8px 14px; background:linear-gradient(90deg,#f0f5ff,transparent); border-left:3px solid #1677ff; border-radius:0 8px 8px 0; margin-bottom:10px; }
        .ud-section-empty { text-align:center; padding:20px; color:#8b93a7; font-size:13px; background:#fafbfe; border-radius:10px; border:1px dashed #e5e9f2; }
        .ud-table { width:100%; border-collapse:separate; border-spacing:0; }
        .ud-table th { background:#f8f9fc; color:#8b93a7; font-size:12px; font-weight:600; padding:8px 12px; text-align:left; border-bottom:1px solid #eef1f6; }
        .ud-table td { padding:8px 12px; font-size:13px; border-bottom:1px solid #f5f6fa; color:#3d4566; }
        .ud-table tr:hover td { background:#fafbfe; }
        .ud-badge { display:inline-block; padding:2px 10px; border-radius:99px; font-size:11px; font-weight:600; }
        .ud-badge-green { background:#e6f4ea; color:#1a7f4b; }
        .ud-badge-yellow { background:#fffbe6; color:#d48806; }
        .ud-badge-red { background:#fff1f0; color:#cf1322; }
        .ud-badge-gray { background:#f5f5f5; color:#999; }
        .ud-btn-sm { padding:4px 10px; font-size:12px; border-radius:6px; border:1px solid #d9dde7; background:#fff; cursor:pointer; transition:all .15s; }
        .ud-btn-sm:hover { border-color:#1677ff; color:#1677ff; }
        .user-row:hover { background:#f8faff; }
        .user-row td { padding:10px 8px; }
        .user-avatar-sm { width:32px; height:32px; border-radius:50%; object-fit:cover; vertical-align:middle; margin-right:8px; border:2px solid #f0f2f8; }
    </style>
</head>
<body>
<div class="top">
...
<div class="main">
    <div class="card">
        <div class="row" style="flex-wrap:wrap;">
            ...
            <button id="tabSensitiveWord" class="btn tab-btn" onclick="loadSensitiveWordList()">敏感词管理</button>
            <button id="tabReportList" class="btn tab-btn" onclick="loadReportList()">待处理举报</button>
            <button id="tabPostList" class="btn tab-btn" onclick="loadPostList()">待审核帖子</button>
        </div>
        ...
        <div id="sensitiveWordBar" class="row" style="display:none;flex-wrap:wrap;gap:10px;align-items:center;">
            <input id="swWordInput" class="input" style="max-width:220px;" placeholder="输入敏感词" maxlength="50" />
            <select id="swLevelInput" class="btn" style="max-width:140px;">
                <option value="1">一般</option>
                <option value="2">严重</option>
            </select>
            <button class="btn btn-primary" onclick="addSensitiveWord()">添加敏感词</button>
            <button class="btn" onclick="loadSensitiveWordList()">刷新</button>
        </div>
...
<script>
    ...
    function setActiveTab(tab){ ['tabPostList','tabReportList','tabAnnList','tabUserList','tabSensitiveWord'].forEach(function(id){ var el = document.getElementById(id); if(el) el.classList.remove('active'); }); if(tab === 'post') document.getElementById('tabPostList').classList.add('active'); else if(tab === 'report') document.getElementById('tabReportList').classList.add('active'); else if(tab === 'ann') document.getElementById('tabAnnList').classList.add('active'); else if(tab === 'user') document.getElementById('tabUserList').classList.add('active'); else if(tab === 'sw') document.getElementById('tabSensitiveWord').classList.add('active'); }
    async function init(){ try{ var profile = await api('/api/admin/profile'); if(profile.code !== 200){ document.getElementById('adminName').innerText = '鉴权失败'; document.getElementById('hint').innerHTML = '<span class="err">登录态无效，请重新登录</span>'; return; } document.getElementById('adminName').innerText = profile.data.username || '管理员'; restoreReportFilters(); await loadUserList(); }catch(e){ document.getElementById('adminName').innerText = '鉴权失败'; document.getElementById('hint').innerHTML = '<span class="err">请求异常，请刷新重试</span>'; } }
    ...
    async function loadPostList(){ setActiveTab('post'); document.getElementById('reportFilters').style.display = 'none'; document.getElementById('userFilters').style.display = 'none'; document.getElementById('annBar').style.display = 'none'; document.getElementById('sensitiveWordBar').style.display = 'none'; document.getElementById('tableWrap').innerHTML = ''; document.getElementById('hint').innerText = '正在加载待审核帖子...'; var data = await api('/api/admin/post/list?status=0&page=1&pageSize=50'); if(data.code !== 200){ document.getElementById('hint').innerHTML = '<span class="err">' + esc(data.msg || '加载失败') + '</span>'; return; } postListCache = (data.data && data.data.list) || []; renderPostTable(postListCache); document.getElementById('hint').innerText = '共 ' + postListCache.length + ' 条待审核帖子'; }
    ...
    async function loadAnnList(){ setActiveTab('ann'); document.getElementById('reportFilters').style.display = 'none'; document.getElementById('userFilters').style.display = 'none'; document.getElementById('annBar').style.display = 'flex'; document.getElementById('sensitiveWordBar').style.display = 'none'; document.getElementById('tableWrap').innerHTML = ''; document.getElementById('hint').innerText = '正在加载公告...'; var data = await api('/api/admin/announcement/listAdmin?page=1&pageSize=50'); if(data.code !== 200){ document.getElementById('hint').innerHTML = '<span class="err">' + esc(data.msg || '加载失败') + '</span>'; return; } annListCache = (data.data && data.data.list) || []; renderAnnTable(annListCache); document.getElementById('hint').innerText = '共 ' + annListCache.length + ' 条公告'; }
    ...
    async function openUserDetail(id){ var data = await api('/api/admin/user/' + id); if(data.code !== 200){ alert(data.msg || '加载失败'); return; } var d = data.data || {}; var isBanned = Number(d.status) === 0; var avatarUrl = d.avatarUrl || 'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png'; var statusHtml = isBanned ? '<span class="ud-status ud-status-ban">已封禁</span>' : '<span class="ud-status ud-status-ok">正常</span>'; var html = '<div class="ud-header"><img class="ud-avatar" src="' + esc(avatarUrl) + '" onerror="this.src=\'https://cube.elemecdn.com/3/7c/3ea6beec64369c2642b92c6726f1epng.png\'" /><div><div class="ud-name">' + esc(d.nickname || '-') + '</div><div class="ud-id">用户ID：' + esc(d.id) + '</div>' + statusHtml + '</div></div>' + '<div class="ud-info-grid"><div class="ud-info-item"><div class="ud-info-label">手机号</div><div class="ud-info-value">' + esc(d.phone || '-') + '</div></div><div class="ud-info-item"><div class="ud-info-label">注册日期</div><div class="ud-info-value">' + esc(formatDateOnly(d.createTime)) + '</div></div><div class="ud-info-item"><div class="ud-info-label">最近活跃</div><div class="ud-info-value">' + esc(formatTime(d.lastActiveTime)) + '</div></div><div class="ud-info-item"><div class="ud-info-label">风险等级</div><div class="ud-info-value">' + esc(d.riskLevel || '低') + '</div></div></div>' + '<div class="ud-section"><div class="ud-section-title">发帖记录</div>' + renderUserPostList(d.posts || []) + '</div>' + '<div class="ud-section"><div class="ud-section-title">评论记录</div>' + renderUserCommentList(d.comments || []) + '</div>' + '<div class="ud-section"><div class="ud-section-title">收藏记录</div>' + renderUserCollectList(d.collects || []) + '</div>'; document.getElementById('userDetailContent').innerHTML = html; document.getElementById('userDetailModalMask').style.display='flex'; }
    function renderUserPostList(list){ if(!list || !list.length) return '<div class="ud-section-empty">暂无发帖记录</div>'; var html = '<table class="ud-table"><thead><tr><th>ID</th><th>标题</th><th>分类</th><th>状态</th><th>操作</th></tr></thead><tbody>'; for(var i=0;i<list.length;i++){ var it = list[i] || {}; var s = Number(it.status); var badge = s===1?'<span class="ud-badge ud-badge-green">已发布</span>':s===0?'<span class="ud-badge ud-badge-yellow">待审核</span>':s===2?'<span class="ud-badge ud-badge-red">已拒绝</span>':s===3?'<span class="ud-badge ud-badge-gray">已下架</span>':'<span class="ud-badge ud-badge-gray">'+esc(it.status)+'</span>'; html += '<tr><td>' + esc(it.id) + '</td><td>' + esc(it.title) + '</td><td>' + esc(it.category || '-') + '</td><td>' + badge + '</td><td><button class="ud-btn-sm" onclick="previewPost(' + Number(it.id) + ')">预览</button></td></tr>'; } return html + '</tbody></table>'; }
    function renderUserCommentList(list){ if(!list || !list.length) return '<div class="ud-section-empty">暂无评论记录</div>'; var html = '<table class="ud-table"><thead><tr><th>ID</th><th>帖子ID</th><th>内容</th><th>时间</th><th>操作</th></tr></thead><tbody>'; for(var i=0;i<list.length;i++){ var it = list[i] || {}; var pid = Number(it.postId || 0); var shortContent = String(it.content || '').length > 30 ? String(it.content).substring(0,30) + '...' : (it.content || '-'); html += '<tr><td>' + esc(it.id) + '</td><td>' + esc(it.postId) + '</td><td title="' + esc(it.content) + '">' + esc(shortContent) + '</td><td>' + esc(formatTime(it.createTime)) + '</td><td>' + (pid > 0 ? '<button class="ud-btn-sm" onclick="previewCommentPost(' + pid + ')">看帖子</button>' : '<span class="muted">-</span>') + '</td></tr>'; } return html + '</tbody></table>'; }
    function renderUserCollectList(list){ if(!list || !list.length) return '<div class="ud-section-empty">暂无收藏记录</div>'; var html = '<table class="ud-table"><thead><tr><th>ID</th><th>标题</th><th>分类</th><th>状态</th><th>操作</th></tr></thead><tbody>'; for(var i=0;i<list.length;i++){ var it = list[i] || {}; var s = Number(it.status); var badge = s===1?'<span class="ud-badge ud-badge-green">已发布</span>':s===0?'<span class="ud-badge ud-badge-yellow">待审核</span>':s===2?'<span class="ud-badge ud-badge-red">已拒绝</span>':s===3?'<span class="ud-badge ud-badge-gray">已下架</span>':'<span class="ud-badge ud-badge-gray">'+esc(it.status)+'</span>'; html += '<tr><td>' + esc(it.id) + '</td><td>' + esc(it.title) + '</td><td>' + esc(it.category || '-') + '</td><td>' + badge + '</td><td><button class="ud-btn-sm" onclick="previewPost(' + Number(it.id) + ')">预览</button></td></tr>'; } return html + '</tbody></table>'; }
    ...
    async function toggleUserStatus(id, currentStatus){ var target = Number(currentStatus) === 0 ? 1 : 0; if(target === 0){ var reason = prompt('请输入封禁原因', '违规操作'); if(reason === null) return; var data = await api('/api/admin/user/status/' + id, 'POST', { status: target, reason: reason, remark: '' }); alert(data.msg || (data.code===200 ? '操作成功' : '操作失败')); loadUserList(); }else{ if(!confirm('确认解封该用户？')) return; var data = await api('/api/admin/user/status/' + id, 'POST', { status: target, reason: '', remark: '' }); alert(data.msg || (data.code===200 ? '操作成功' : '操作失败')); loadUserList(); } }
    function viewPostDetailByReport(postId){ previewPost(postId); }
    var swListCache = [];
    async function loadSensitiveWordList(){ setActiveTab('sw'); document.getElementById('reportFilters').style.display='none'; document.getElementById('userFilters').style.display='none'; document.getElementById('annBar').style.display='none'; document.getElementById('sensitiveWordBar').style.display='flex'; document.getElementById('tableWrap').innerHTML=''; document.getElementById('hint').innerText='正在加载敏感词...'; var data = await api('/api/admin/sensitive-word/list'); if(data.code !== 200){ document.getElementById('hint').innerHTML = '<span class="err">' + esc(data.msg || '加载失败') + '</span>'; return; } swListCache = data.data || []; renderSensitiveWordTable(swListCache); document.getElementById('hint').innerText = '共 ' + swListCache.length + ' 个敏感词'; }
    function renderSensitiveWordTable(list){ if(!list || !list.length){ document.getElementById('tableWrap').innerHTML = '<div class="muted">暂无敏感词</div>'; return; } var html = '<table class="table"><thead><tr><th>ID</th><th>敏感词</th><th>级别</th><th>添加时间</th><th>操作</th></tr></thead><tbody>'; for(var i=0;i<list.length;i++){ var it = list[i] || {}; var levelBadge = Number(it.level) === 2 ? '<span style="background:#fff1f0;color:#cf1322;padding:2px 10px;border-radius:99px;font-size:12px;font-weight:600;">严重</span>' : '<span style="background:#fffbe6;color:#d48806;padding:2px 10px;border-radius:99px;font-size:12px;font-weight:600;">一般</span>'; html += '<tr><td>' + esc(it.id) + '</td><td style="font-weight:600;">' + esc(it.word) + '</td><td>' + levelBadge + '</td><td>' + esc(formatTime(it.createTime)) + '</td><td><button class="btn" onclick="deleteSensitiveWord(' + Number(it.id) + ')">删除</button></td></tr>'; } html += '</tbody></table>'; document.getElementById('tableWrap').innerHTML = html; }
    async function addSensitiveWord(){ var word = document.getElementById('swWordInput').value.trim(); var level = document.getElementById('swLevelInput').value; if(!word){ alert('请输入敏感词'); return; } var data = await api('/api/admin/sensitive-word/add?word=' + encodeURIComponent(word) + '&level=' + encodeURIComponent(level), 'POST'); if(data.code !== 200){ alert(data.msg || '添加失败'); return; } document.getElementById('swWordInput').value = ''; loadSensitiveWordList(); }
    async function deleteSensitiveWord(id){ if(!confirm('确认删除该敏感词吗？')) return; var data = await api('/api/admin/sensitive-word/delete/' + id, 'DELETE'); alert(data.msg || (data.code===200 ? '删除成功' : '删除失败')); loadSensitiveWordList(); }
