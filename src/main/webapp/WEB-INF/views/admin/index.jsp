...
<div class="main">
    <div class="card">
        ...
        <div id="userFilters" class="row" style="display:none;flex-wrap:wrap;gap:10px;">
            ...
            <select id="userRiskFilter" class="btn" style="max-width:180px;">
                <option value="">全部风险</option>
                <option value="normal">正常</option>
                <option value="post">发帖过多</option>
                <option value="comment">评论过密</option>
                <option value="report">举报过多</option>
            </select>
...
<script>
    ...
    function resetUserFilters(){ document.getElementById('userKeyword').value=''; document.getElementById('userRiskFilter').value=''; loadUserList(); }
    async function loadUserList(){ setActiveTab('user'); document.getElementById('reportFilters').style.display='none'; document.getElementById('annBar').style.display='none'; document.getElementById('userFilters').style.display='flex'; document.getElementById('tableWrap').innerHTML=''; document.getElementById('hint').innerText='正在加载用户...'; var keyword = document.getElementById('userKeyword').value.trim(); var riskType = document.getElementById('userRiskFilter').value; var qs = ['page=1','pageSize=50']; if(keyword) qs.push('keyword=' + encodeURIComponent(keyword)); if(riskType) qs.push('riskType=' + encodeURIComponent(riskType)); var data = await api('/api/admin/user/list?' + qs.join('&')); if(data.code !== 200){ document.getElementById('hint').innerHTML = '<span class="err">' + esc(data.msg || '加载失败') + '</span>'; return; } userListCache = (data.data && data.data.list) || []; renderUserTable(userListCache); document.getElementById('hint').innerText = '共 ' + userListCache.length + ' 个用户'; }
    function renderUserTable(list){ if(!list || !list.length){ document.getElementById('tableWrap').innerHTML = '<div class="muted">暂无用户</div>'; return; } var html = '<table class="table"><thead><tr><th>ID</th><th>昵称</th><th>手机号</th><th>注册时间</th><th>最近活跃</th><th>发帖(1h)</th><th>评论(10m)</th><th>举报(7d)</th><th>风险</th><th>操作</th></tr></thead><tbody>'; for(var i=0;i<list.length;i++){ var it = list[i] || {}; var risk = it.riskLevel || '正常'; var riskBadge = '<span style="padding:2px 10px;border-radius:99px;background:' + (risk === '正常' ? '#f0f5ff' : '#fff7e6') + ';color:' + (risk === '正常' ? '#1677ff' : '#d46b08') + ';font-size:12px;">' + esc(risk) + '</span>'; html += '<tr><td>' + esc(it.id) + '</td><td>' + esc(it.nickname || '-') + '</td><td>' + esc(it.phone || '-') + '</td><td>' + esc(formatTime(it.createTime)) + '</td><td>' + esc(formatTime(it.lastActiveTime)) + '</td><td>' + esc(it.postCount1h || 0) + '</td><td>' + esc(it.commentCount10m || 0) + '</td><td>' + esc(it.reportCount7d || 0) + '</td><td>' + riskBadge + '</td><td><button class="btn btn-primary" onclick="openUserDetail(' + Number(it.id) + ')">详情</button></td></tr>'; } html += '</tbody></table>'; document.getElementById('tableWrap').innerHTML = html; }
    async function openUserDetail(id){ var data = await api('/api/admin/user/' + id); if(data.code !== 200){ alert(data.msg || '加载失败'); return; } var d = data.data || {}; var riskText = (d.riskTags || []).join('，') || '正常'; var html = '<div class="card" style="box-shadow:none;border:1px solid #eef1f6;margin-bottom:12px;"><div class="kv"><b>昵称：</b>' + esc(d.nickname || '-') + '</div><div class="kv"><b>手机号：</b>' + esc(d.phone || '-') + '</div><div class="kv"><b>注册时间：</b>' + esc(formatTime(d.createTime)) + '</div><div class="kv"><b>最近活跃：</b>' + esc(formatTime(d.lastActiveTime)) + '</div><div class="kv"><b>风险：</b>' + esc(riskText) + '</div><div class="kv"><b>风控统计：</b>发帖1h ' + esc(d.postCount1h || 0) + '｜评论10m ' + esc(d.commentCount10m || 0) + '｜举报7d ' + esc(d.reportCount7d || 0) + '</div></div>' + '<div class="user-tabs" style="display:flex;gap:8px;flex-wrap:wrap;margin-bottom:12px;"><button class="btn" onclick="switchUserTab(\'posts\')">发帖</button><button class="btn" onclick="switchUserTab(\'comments\')">评论</button><button class="btn" onclick="switchUserTab(\'collects\')">收藏</button></div>' + '<div id="userTabPosts">' + renderUserPostList(d.posts || []) + '</div>' + '<div id="userTabComments" style="display:none;">' + renderUserCommentList(d.comments || []) + '</div>' + '<div id="userTabCollects" style="display:none;">' + renderUserCollectList(d.collects || []) + '</div>'; document.getElementById('userDetailContent').innerHTML = html; document.getElementById('userDetailModalMask').style.display='flex'; window.__currentUserDetail = d; }
    function switchUserTab(tab){ var posts = document.getElementById('userTabPosts'); var comments = document.getElementById('userTabComments'); var collects = document.getElementById('userTabCollects'); if(!posts || !comments || !collects) return; posts.style.display = tab === 'posts' ? 'block' : 'none'; comments.style.display = tab === 'comments' ? 'block' : 'none'; collects.style.display = tab === 'collects' ? 'block' : 'none'; }
    ...
    function renderUserCommentList(list){ if(!list || !list.length) return '<div class="muted">暂无评论</div>'; var html = '<table class="table"><thead><tr><th>ID</th><th>帖子ID</th><th>内容</th><th>时间</th><th>操作</th></tr></thead><tbody>'; for(var i=0;i<list.length;i++){ var it = list[i] || {}; html += '<tr><td>' + esc(it.id) + '</td><td>' + esc(it.postId) + '</td><td>' + esc(it.content) + '</td><td>' + esc(formatTime(it.createTime)) + '</td><td><button class="btn btn-primary" onclick="previewPost(' + Number(it.postId) + ')">看帖子</button></td></tr>'; } return html + '</tbody></table>'; }
